...
```{r}
# Create a sequence of dates for each survey date with lags
...
```{r}
# Create a sequence of dates for each survey date with 7-day lags
temperature_lags <- pss_simple %>%
  filter(diffdays < 0) %>%
  select(mstudyid, vname, survey_date) %>%
  mutate(start_date = survey_date - 7) %>%  # Define the start of the 7-day window
  rowwise() %>%
  mutate(lag_dates = list(seq.Date(start_date, survey_date, by = "day"))) %>%  # Create a list of dates for each survey
  unnest(lag_dates) %>%  # Expand rows for each date in the 7-day range
  rename(date = lag_dates) %>%
